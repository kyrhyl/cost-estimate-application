...
interface DUPATemplate {
  ...
  notes: string;
  laborTemplate: LaborEntry[];
  equipmentTemplate: EquipmentEntry[];
  materialTemplate: MaterialEntry[];
...
export default function EditDUPATemplatePage() {
  ...
  const fetchTemplate = async () => {
    try {
      ...
      if (data.success) {
        const template: DUPATemplate = data.data;
        ...
        setOcmPercentage(template.ocmPercentage || 10);
        setCpPercentage(template.cpPercentage || 8);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        router.push(`/dupa-templates/${params.id}`);
      } else {
